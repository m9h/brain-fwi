"""Staged transmission-traveltime pose recovery: translation -> rotation -> joint.

Joint gradient descent on the transmission TT misfit (job 1692) solved TRANSLATION to
sub-voxel (|dt| 0.23-0.42 vox from origin) but ROTATION went the wrong way (rz -> -3..-5
vs +5): the rz bowl is ~10x weaker than translation, so coupling misleads rz while tx,ty
still move. The 1D scan proved rz has a clean min at +5 ONCE translation is at truth.
Fix = decouple: Stage 1 optimise (tx,ty) with rz frozen; Stage 2 optimise rz with (tx,ty)
frozen on its now-clean bowl; Stage 3 joint refine. Single-warp consistent forward.
"""
...
print(f"staged transmission pose: brain={'TRUE' if TRUE_BRAIN else 'generic'}, {nsteps} steps, {POSE_SHOTS} shots", flush=True)
...
vg = eqx.filter_jit(jax.value_and_grad(loss))
def overlap(q):
...
def descend(p, active, steps, lr, tag):
    opt = optax.adam(optax.cosine_decay_schedule(lr, steps))   # decay -> settle, no overshoot/oscillation
    st = opt.init(p); t0 = time.time()
    for i in range(steps):
        l, g = vg(p)
        g = {k: (g[k] if k in active else jnp.zeros_like(g[k])) for k in g}
        up, st = opt.update(g, st); p = optax.apply_updates(p, up)
        if (i + 1) % 5 == 0 or i == steps - 1:
            print(f"  [{tag}] step {i+1}: TT {float(l):.3e} t=({float(p['tx']):.2f},{float(p['ty']):.2f}) "
                  f"rz={float(p['rz']):.2f} overlap {overlap(p):.0f}% "
                  f"err|dt|={np.hypot(float(p['tx'])-4,float(p['ty'])-2):.2f} |drz|={abs(float(p['rz'])-5):.2f} "
                  f"({time.time()-t0:.0f}s)", flush=True)
    return p

# coarse forward-only (tx,ty) grid at rz=0 -> robust translation init. The cold-origin
# gradient is weak/bistable for the generic brain (the mismatch floor lifts truth toward the
# tx~-1 bump), so plain descent from origin can drift into the wrong basin (job 1695: 45%).
# The transmission translation bowl is smooth, so a coarse grid reliably lands the right basin.
mf = eqx.filter_jit(loss)
_g = lambda tx, ty: {"tx": jnp.asarray(tx, jnp.float32), "ty": jnp.asarray(ty, jnp.float32), "rz": jnp.asarray(0.0, jnp.float32)}
GTX = np.arange(-6, 7, 3.0); GTY = np.arange(-6, 7, 3.0); t0 = time.time(); best = (1e30, 0.0, 0.0)
for gtx in GTX:
    for gty in GTY:
        m = float(mf(_g(gtx, gty)))
        if m < best[0]:
            best = (m, float(gtx), float(gty))
print(f"  prealign grid {len(GTX)*len(GTY)} evals: best (tx,ty)=({best[1]:.0f},{best[2]:.0f}) misfit {best[0]:.3e} "
      f"({time.time()-t0:.0f}s)", flush=True)
p = {"tx": jnp.array(best[1]), "ty": jnp.array(best[2]), "rz": jnp.array(0.0)}
print(f"  init overlap {overlap(p):.0f}% (post-prealign)", flush=True)
# grid-seed BOTH weak-gradient DOFs (translation basin via the 2D grid above; rotation via a 1D
# grid below), then gradient-polish. The generic-brain rz gradient is real but weak (job 1696: rz
# stalled at ~0.8 vs +5), so a forward-only rz grid at the locked translation is far more robust
# than descent on the shallow rz bowl.
p = descend(p, {"tx", "ty"}, 20, 0.4, "1:translation")
RZg = np.arange(-10, 16, 2.0); brz = (1e30, 0.0); t0 = time.time()
for grz in RZg:
    m = float(mf({"tx": p["tx"], "ty": p["ty"], "rz": jnp.asarray(grz, jnp.float32)}))
    if m < brz[0]:
        brz = (m, float(grz))
print(f"  rz-grid {len(RZg)} evals: best rz={brz[1]:.0f} misfit {brz[0]:.3e} ({time.time()-t0:.0f}s)", flush=True)
p = {"tx": p["tx"], "ty": p["ty"], "rz": jnp.array(brz[1])}
print(f"  post rz-grid overlap {overlap(p):.0f}%", flush=True)
p = descend(p, {"tx", "ty", "rz"}, 20, 0.2, "2:joint")
ov = overlap(p)
print(f"FINAL: t=({float(p['tx']):.2f},{float(p['ty']):.2f},0) rz={float(p['rz']):.2f}deg (true 4,2,5) "
      f"OVERLAP {ov:.0f}% => pose {'LANDED' if ov >= 90 else 'PARTIAL' if ov >= 75 else 'did NOT land'}", flush=True)
np.save("/tmp/mofi_tt_staged_pose.npy", np.array([float(p["tx"]), float(p["ty"]), float(p["rz"])]))
